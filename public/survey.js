...
(function () {
  ...
  // Render a visible, diagnosable error state instead of a blank page or a
  // wrong-language survey. window.survey stays unset so the automated walker
  // never mistakes this for a rendered survey.
  function renderError(message) {
    ...
    if (container) {
      ...
      box.style.cssText =
        "margin:2em auto;max-width:40em;padding:1em 1.5em;border:2px solid #c0392b;" +
        "border-radius:6px;color:#c0392b;background:#fdf3f2;font-family:sans-serif;";
      box.textContent = "Survey QA error: " + message;
  ...
  function renderModel(json) {
    ...
    if (typeof Survey === "undefined" || typeof Survey.Model !== "function") {
      renderError("the SurveyJS library failed to load (CDN unreachable or blocked); the survey cannot be rendered.");
      return;
    }
  ...
  function init() {
    ...
    // Localized models (with their own seeded errors) are generated by
        ...
        if (models && models[lang]) {
          ...
          renderError(
            'no localized survey model exists for lang="' + lang + '"' +
              " (was scripts/gen-survey.mjs run and /survey-models.json deployed?)." +
              " Refusing to render the English survey in its place."
          );
        }
      })
      .catch(function (err) {
        renderError(
          'failed to load /survey-models.json for lang="' + lang + '": ' +
