#!/usr/bin/env node
/**
 * plan-paths.mjs — the TWO-TIER COVERAGE PLANNER.
 *
 * Implements the owner's binding coverage ruling:
 *
 *   TIER 1 (FLOOR, contractual)   a path set that provably exercises EVERY obligation.
 *                                 Must complete. Set-cover: prefer fewer paths, but never
 *                                 at the cost of leaving an obligation uncovered.
 *   TIER 2 (DIRECTED EXPLORATION) a risk-ranked queue derived MECHANICALLY from the
 *                                 checklist. May only ADD findings. Runs until the queue
 *                                 empties or a cap hits.
 *
 * INVARIANTS (enforced in code, restated in the emitted plan):
 *   I1. DENOMINATOR LOCK. The denominator is the checklist, and only the checklist.
 *       Tier 2 adds findings and observations; it can never add or remove an obligation.
 *   I2. NO RE-VERIFICATION. Verification is the expensive step; walking is not. An
 *       exploration observation EQUIVALENT to one already judged is discarded before it
 *       reaches a judge (equivalence key defined in `outcome_protocol`).
 *   I3. DEDUPE BY ROOT CAUSE, NOT BY SYMPTOM. Every planned observation carries a
 *       `root_cause_key`; candidate findings are grouped on it before verification, and
 *       one verification serves the whole group.
 *   I4. PATH-DEPENDENT BEHAVIOUR IS A FIRST-CLASS OUTCOME. If the same obligation behaves
 *       differently on two paths, that is not a contradiction to be overwritten by
 *       whichever ran last — it is emitted as PATH_DEPENDENT_BEHAVIOUR carrying both
 *       observations. It is usually more serious than a plain failure.
 *
 * BLINDNESS: this planner is a pure function of the coverage contract. It does not read
 * the site, its JS, or any answer key. It does not read the questionnaire either — the
 * extractor already did, and re-reading it here would let the planner smuggle in
 * obligations the contract does not carry (violating I1). The only other files it touches
 * are the extractor's own chunk envelopes and its CHUNKS manifest, and only to audit
 * whether the denominator is complete.
 *
 * Usage:
 *   node plan-paths.mjs [--run <dir>] [--checklist <file>] [--out <file>]
 *                       [--chunks <dir>] [--no-provisional]
 *                       [--max-queue N] [--per-class-cap N]
 *                       [--cost-per-step 0.00024] [--seconds-per-step 3]
 */

import { readFileSync, writeFileSync, existsSync, readdirSync, mkdirSync } from 'node:fs';
import { join, dirname, resolve, basename } from 'node:path';
import { fileURLToPath } from 'node:url';
import { loadContract, emptyContract, hashContract, CONTRACT_KIND } from './lib/contract.mjs';
import { buildUncontractedProbes, rebaseInfo, pathSignature } from './lib/plan-augment.mjs';
...
function parseArgs(argv) {
  const a = {
    run: resolve(REPO, 'pipeline', 'runs', 't1-easy'),
    checklist: null, out: null, chunks: null,
    provisional: true,
    maxQueue: 400, perClassCap: 60,
    costPerStep: 0.00024, secondsPerStep: 3,
    quiet: false,
  };
  for (let i = 2; i < argv.length; i++) {
    const k = argv[i], v = argv[i + 1];
    switch (k) {
      case '--run': a.run = resolve(v); i++; break;
      case '--checklist': a.checklist = resolve(v); i++; break;
      case '--out': a.out = resolve(v); i++; break;
      case '--chunks': a.chunks = resolve(v); i++; break;
      case '--no-provisional': a.provisional = false; break;
      case '--max-queue': a.maxQueue = Number(v); i++; break;
      case '--per-class-cap': a.perClassCap = Number(v); i++; break;
      case '--cost-per-step': a.costPerStep = Number(v); i++; break;
      case '--seconds-per-step': a.secondsPerStep = Number(v); i++; break;
      case '--quiet': a.quiet = true; break;
      default: if (k.startsWith('--')) throw new Error(`unknown flag ${k}`);
    }
  }
  a.checklist ??= join(a.run, 'checklist.json');
  a.out ??= join(a.run, 'plan.json');
  a.chunks ??= join(a.run, 'chunks');
  return a;
}

// ===========================================================================
// 1. Contract acquisition (with an honest provisional fallback)
// ===========================================================================

function acquireContract(args) {
  const primary = loadContract(args.checklist);
  if (primary.ok) {
    const audit = auditChunkDir(args.chunks);
    const warnings = [...primary.warnings];
    // The checklist is authoritative, but if the extractor's own manifest lists chunks that
    // never landed under the expected tag, say so: the denominator may be short and nothing
    // downstream would ever notice.
    // Before crying "short denominator", check the CONTRACT rather than the filenames: a
    // chunk that was split (B2 -> B2A + B2B) still contributed its obligations, and a false
    // "coverage is an upper bound" caveat in the report is its own kind of dishonesty.
    const contractTags = new Set(primary.contract.obligations.map((o) => String(o.source_chunk ?? o.chunk_id ?? '')).filter(Boolean));
    const stillMissing = (audit.missing || []).filter((m) => {
      const tag = String(m).split(':').pop();
      return ![...contractTags].some((t) => t === tag || t.startsWith(tag));
    });
    audit.missing_but_present_in_contract = (audit.missing || []).filter((m) => !stillMissing.includes(m));
    if (stillMissing.length) warnings.push(`chunk audit: the extractor manifest declares chunk(s) ${stillMissing.join(', ')} whose obligations appear nowhere in the checklist. The denominator is short by whatever they carried, and every coverage figure is an upper bound.`);
    else if (audit.missing?.length) warnings.push(`chunk audit: manifest chunk(s) ${audit.missing.join(', ')} did not land under that exact tag, but the checklist does carry obligations from a split of them (${[...contractTags].sort().join(', ')}), so the denominator is not short on that account.`);
    if (audit.failed?.length) warnings.push(`chunk audit: ${audit.failed.length} chunk file(s) unusable: ${audit.failed.join(', ')}.`);
    return { contract: primary.contract, status: 'authoritative', warnings, blockers: primary.blockers, source: args.checklist, chunkAudit: audit };
  }
  const audit = auditChunkDir(args.chunks);
  if (!args.provisional || !audit.chunkPayloads.length) {
    return { contract: primary.contract, status: 'blocked', warnings: primary.warnings, blockers: primary.blockers, source: args.checklist, chunkAudit: audit };
  }

  const merged = emptyContract();
  const seen = new Set();
  const warnings = [...primary.warnings];
  for (const { tag, payload } of audit.chunkPayloads) {
    for (const key of ['obligations', 'ambiguities', 'unverifiable_from_browser']) {
      ...
      for (const item of arr) {
        ...
        const id = item.id || `${key}:${tag}:${merged[key].length}`;
        if (seen.has(`${key}::${id}`)) { warnings.push(`duplicate ${key} id "${id}" — kept first.`); continue; }
        seen.add(`${key}::${id}`);
        merged[key].push({ ...item, id, chunk_id: item.chunk_id ?? payload.chunk_id ?? tag ?? null });
      }
    }
  }
  merged.provenance = { kind: CONTRACT_KIND, source: args.chunks, denominatorAuthority: 'provisional-chunk-reconstruction', chunkCount: audit.chunkPayloads.length };
  merged.contractHash = hashContract(merged);

  const blockers = [...primary.blockers];
  if (audit.missing.length) {
    blockers.push({
      code: 'CONTRACT_INCOMPLETE', severity: 'blocking',
      detail: `The extractor's chunk manifest declares ${audit.expected} chunks; ${audit.missing.length} are absent or unusable: ${audit.missing.join(', ')}.`,
      consequence: 'The denominator is short by whatever those chunks would have contributed. Any coverage percentage computed against it is an upper bound, not a truth. Re-run the planner once extraction completes.',
    });
  }
  return { contract: merged, status: 'provisional', warnings, blockers, source: args.chunks, chunkAudit: audit };
}

function auditChunkDir(dir) {
  const audit = { dir, expected: null, expectedSource: null, manifest: null, present: [], missing: [], failed: [], chunkPayloads: [] };
  if (!existsSync(dir)) return audit;
  const driver = join(dir, 'extract-chunk.mjs');
  if (existsSync(driver)) {
    try {
      const src = readFileSync(driver, 'utf8');
      const tags = [...src.matchAll(/\{\s*n:\s*(\d+)\s*,\s*tag:\s*"([^"]+)"/g)].map((m) => ({ n: Number(m[1]), tag: m[2] }));
      if (tags.length) { audit.expected = tags.length; audit.expectedSource = 'extract-chunk.mjs CHUNKS manifest'; audit.manifest = tags; }
    } catch { /* advisory only */ }
  }
  for (const f of readdirSync(dir).filter((f) => /^chunk-\d+\.json$/.test(f)).sort()) {
    let env;
    try { env = JSON.parse(readFileSync(join(dir, f), 'utf8')); } catch { audit.failed.push(f); continue; }
    const tag = String(env.tag ?? env.chunk ?? f);
    if (env.status && env.status !== 'ok') { audit.failed.push(`${f} (status=${env.status})`); continue; }
    const payload = env.parsed && typeof env.parsed === 'object' ? env.parsed : (env.obligations ? env : null);
    if (!payload) { audit.failed.push(`${f} (no parsed payload)`); continue; }
    audit.present.push({ file: f, n: env.chunk ?? null, tag, obligations: (payload.obligations || []).length, incomplete: env.incomplete ?? null });
    audit.chunkPayloads.push({ tag, payload });
  }
  if (audit.manifest) {
    const have = new Set(audit.present.map((p) => p.tag));
    audit.missing = audit.manifest.filter((m) => !have.has(m.tag)).map((m) => `${m.n}:${m.tag}`);
    // A tag present under a variant name (B2 -> B2A) is a PARTIAL chunk, not a present one.
    audit.tag_mismatches = audit.present.filter((p) => !audit.manifest.some((m) => m.tag === p.tag)).map((p) => p.tag);
  }
  return audit;
}
...
function main() {
  ...
  const acquired = acquireContract(args);
  const contract = acquired.contract;
  const log = (...a) => { if (!args.quiet) console.log(...a); };

  if (!contract.obligations.length) {
    const plan = {
      kind: 'coverage-plan/two-tier-v1', generated_at: new Date().toISOString(), run: basename(args.run),
    ...
    writeOut(args.out, plan);
    log(`plan.json written (BLOCKED — no denominator): ${args.out}`);
    return plan;
  }
  ...
  const cfg = {
    maxQueue: args.maxQueue, perClassCap: args.perClassCap,
    costPerStep: args.costPerStep, secondsPerStep: args.secondsPerStep,
  ...
  // Every walk gets a stable signature BEFORE anything compares plans: it is what lets a
  // re-plan keep observations already collected on an identical walk instead of paying to
  // walk them again.
  ...
  // Probes for requirements the DOCUMENT imposes but the CHECKLIST does not carry, and the
  // record of any re-base onto a different contract. Both live outside the denominator.
  const gapProbes = buildUncontractedProbes(model, contract, cfg);
  const rebase = rebaseInfo(args.out, contract, floor);
  ...
  const plan = {
    ...
    run: basename(args.run),
    status: acquired.status === 'authoritative' ? (hardUncovered.length ? 'OK-WITH-GAPS' : 'OK') : 'PROVISIONAL',
    planner: {
      file: 'pipeline/planner/plan-paths.mjs',
      reads: ['checklist.json (or the extractor chunk envelopes)', "the extractor's CHUNKS manifest — solely to audit whether the denominator is complete"],
      ...
      model_calls: 0,
      model_call_note: 'Planning makes NO model calls at all — it is mechanical, so it is reproducible, auditable, and costs nothing against the Workers AI allowance. The budget belongs to navigation and judging.',
    },
    ...
    outcome_protocol: {
      ...
      dedupe: {
        ...
        key: 'mechanism@anchor_question, mechanism ∈ ' + Object.values(MECHANISM).join(', '),
  ...
  writeOut(args.out, plan);
  log(`floor: ${floor.paths.length} paths, ${floor.witnessOf.size}/${contract.obligations.length} witnessed, ${floor.uncovered.length} uncovered (${hardUncovered.length} hard)`);
  log(`exploration: ${exploration.queue.length} entries ${JSON.stringify(byClass)}`);
  if (rebase) log(`rebase: contract changed — ${rebase.counts.before} -> ${rebase.counts.now} obligations (+${rebase.counts.added}/-${rebase.counts.removed}); ${rebase.floor_paths_identical_to_prior.length} floor path(s) reusable, ${rebase.floor_paths_new_or_changed.length} to walk`);
  log(`estimate: ${floorSteps + expSteps + gapSteps} steps, $${plan.estimate.total.cost_usd}, ~${plan.estimate.total.wall_clock_minutes} min serial`);
