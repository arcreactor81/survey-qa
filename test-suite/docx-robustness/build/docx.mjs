...
export function extractDocxText(data) {
    ...
    if (bytes.byteLength > MAX_ARCHIVE_BYTES) {
        throw new Error(`extractDocxText: .docx archive is ${bytes.byteLength} bytes, above the ${MAX_ARCHIVE_BYTES}-byte limit.`);
    }
    ...
    try {
        entries = unzipSync(bytes, {
            filter: (info) => {
                ...
                if (info.name !== DOCUMENT_XML_PATH)
                    return false;
        ...
        throw new Error(`extractDocxText: input is not a readable .docx (ZIP) archive: ${describeError(err)}`);
    }
    const documentXml = entries[DOCUMENT_XML_PATH];
    if (!documentXml) {
        if (oversizedDeclared > 0) {
            throw new Error(`extractDocxText: "${DOCUMENT_XML_PATH}" declares ${oversizedDeclared} decompressed bytes, above the ${MAX_DOCUMENT_XML_BYTES}-byte limit; refusing to decompress.`);
        }
        const preview = entryNames.slice(0, 10).join(", ");
        const suffix = entryNames.length === 0
    ...
    if (documentXml.byteLength > MAX_DOCUMENT_XML_BYTES) {
        throw new Error(`extractDocxText: decompressed "${DOCUMENT_XML_PATH}" is ${documentXml.byteLength} bytes, above the ${MAX_DOCUMENT_XML_BYTES}-byte limit.`);
    }
    ...
    if (blocks.length === 0) {
        if (xml.trim().length === 0) {
            ...
            throw new Error(`extractDocxText: "${DOCUMENT_XML_PATH}" is empty (no XML content); ` +
                "the .docx contains no document body to compare against.");
        }
        throw new Error(`extractDocxText: no paragraphs could be parsed from "${DOCUMENT_XML_PATH}" (${xml.length} chars of XML); ` +
 ...
 */
function decodeDocumentXml(bytes) {
    let label = "utf-8";
    if (bytes.length >= 2) {
...
function buildSyntax(prefix) {
    ...
    return {
        ...
        runTokenSrc: `<${p}t(?=[\\s/>])[^>]*\\/>|<${p}t(?=[\\s>])[^>]*>([\\s\\S]*?)<\\/${p}t>|` +
            `<${p}tab(?=[\\s/>])[^>]*\\/>|<${p}(?:br|cr)(?=[\\s/>])[^>]*\\/>`,
    };
}
 ...
 */
function stripMarkupCompatibilityFallback(xml) {
 ...
 */
function neutralizeTextBoxParagraphs(xml, wml) {
 ...
 */
function extractBlocks(xml, wml) {
 ...
 */
function extractTableRows(tableXml, wml) {
    const rows = [];
...
function extractParagraphText(paragraphXml, wml) {
    const parts = [];
