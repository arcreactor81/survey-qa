...
function assertNestedReceipt(value) { if (!exact(value.request, ["method", "model", "origin", "path"]) || value.request.method !== "GET" || value.request.model !== "grok-4.6" || value.request.origin !== "https://api.x.ai" || value.request.path !== "/v1/language-models/grok-4.6" || !exact(value.model, ["aliases", "created", "fingerprint", "id", "inputModalities", "object", "outputModalities", "ownedBy", "version"]) || !Number.isSafeInteger(value.model.created) || !strings(value.model.aliases) || !strings(value.model.inputModalities) || !strings(value.model.outputModalities) || typeof value.model.fingerprint !== "string" || typeof value.model.version !== "string" || !exact(value.pricing, ["base", "longContext", "searchTicksPerSearch", "unit", "usdTicksPerUsd"]) || value.pricing.unit !== "usd-ticks-per-token" || value.pricing.usdTicksPerUsd !== 10000000000 || !Number.isSafeInteger(value.pricing.searchTicksPerSearch) || value.pricing.searchTicksPerSearch < 0 || !validRates(value.pricing.base) || !validLongTier(value.pricing.longContext)) throw new Error("attestation receipt nested fields are invalid"); }
