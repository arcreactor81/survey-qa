...
function catalogue(overrides = {}) { return { id: "grok-4.5", object: "model", owned_by: "xai", created: 1, fingerprint: "fp_fixture", version: "1.0", aliases: ["grok-4.5-canonical"], input_modalities: ["text", "image"], output_modalities: ["text"], prompt_text_token_price: 12500, cached_prompt_text_token_price: 2000, prompt_image_token_price: 12500, completion_text_token_price: 25000, prompt_text_token_price_long_context: 25000, cached_prompt_text_token_price_long_context: 0, completion_text_token_price_long_context: 50000, long_context_threshold: 200000, search_price: 0, ...overrides }; }
...
suite("GROK RATE ATTESTATION", () => {
  test("fixed exact-model catalogue becomes a sanitised tick-exact receipt", async () => {
    ...
    assertEq(receipt.request.origin, "https://api.x.ai"); assertEq(receipt.request.path, "/v1/language-models/grok-4.5");
    assertEq(receipt.model.id, "grok-4.5"); assertEq(receipt.pricing.base.inputTextUsdPerMtok, "1.25");
    assertEq(receipt.pricing.base.cachedInputTextUsdPerMtok, "0.2"); assertEq(receipt.pricing.longContext.effectiveRates.cachedInputTextUsdPerMtok, "0.2"); assertEq(receipt.pricing.longContext.effectiveRates.outputTextUsdPerMtok, "5"); assertEq(receipt.model.aliases[0], "grok-4.5-canonical");
  });
  test("fixed request has one exact GET, redirect error, and never lets a caller select a model", async () => {
    ...
    assertEq(calls.length, 1); assertEq(calls[0].url, "https://api.x.ai/v1/language-models/grok-4.5"); assertEq(calls[0].init.method, "GET"); assertEq(calls[0].init.redirect, "error"); assertEq(receipt.model.ownedBy, "xai");
  });
  test("aliases, redirects, unknown keys, duplicate keys, and wrong identity fail closed", async () => {
    ...
    await assertThrows(() => Promise.resolve(m.parseGrokRateCatalogue(JSON.stringify(catalogue({ id: "grok-4.6" })), AT)), "RATE_CATALOGUE_IDENTITY_INVALID");
    ...
    await assertThrows(() => Promise.resolve(m.parseGrokRateCatalogue('{"id":"grok-4.5","id":"grok-4.5"}', AT)), "RATE_CATALOGUE_JSON_INVALID");
  ...
  test("collector rejects same-allowed duplicate keys at every receipt nesting level and emits no bytes", async () => {
    ...
    const malicious = [
      ...
      text.replace('"model":{"id":"grok-4.5"', '"model":{"id":"grok-4.5","id":"grok-4.5"'),
