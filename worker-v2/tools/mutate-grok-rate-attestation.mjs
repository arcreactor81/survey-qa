...
await runMutantSuite({ title: "GROK RATE ATTESTATION MUTANTS", filter: "GROK RATE ATTESTATION", mutants: [
  { name: "accepts another model identity", breaks: "a different model can lend its price to grok-4.6", file: FILE, find: "if (id !== EXACT_GROK_MODEL || object !== \"model\" || ownedBy !== EXACT_XAI_OWNER)", replace: "if (false)", kills: ["aliases, redirects, unknown keys, duplicate keys, and wrong identity fail closed"] },
