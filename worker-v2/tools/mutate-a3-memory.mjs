#!/usr/bin/env node
/**
 * A3b — EVIDENCE THAT THE MEMORY-SAFE JUDGING TESTS CAN FAIL.
 *
 ...
 *      so step-slot and accessibility JSONs are engine-read descriptors instead of hash-
 *      verify-only. The step-exclusion test must kill this because step-slot and
 *      accessibility JSONs would land in engineRead instead of preVerifiedHashes.
 *
 ...
 *
 *   3. MANIFEST COMPUTED OVER UNFILTERED SET: the catalogueDescriptors are not passed to the
 *      authority, so only pre-verified hashes are in the manifest check. The end-to-end test
 ...
 *      deleted, so a session from a path family outside (FLOOR|EXP|TD|T\d) never produces
 *      a descriptor. The unfamiliar-path-family test must kill this.
 *
...
const MUTANTS = [
  // ------------------------------------------------- bounded residency
  {
    name: "the engine-read filter is widened to all artifacts — everything becomes a descriptor",
    breaks:
      "without the narrowed filter, every artifact becomes an engine-read descriptor and " +
      "nothing joins the hash-verify-only stream, which breaks the step-exclusion assertion",
    file: INPUTS,
    ...
    kills: ["peak residency is bounded by session descriptors + one batch, not the full set"],
  },
  ...
  // ------------------------------------------------- manifest coverage
  {
    name: "catalogueDescriptors are not passed to the judge — only pre-verified hashes are in the manifest",
    breaks:
      "without the engine-read descriptors in catalogueDescriptors the authority sees only " +
      "the hash-verify-only set and reports every session JSON as CITED_ARTIFACT_MISSING, " +
      "clearing manifestComplete",
    file: VERDICTS,
    find: "    catalogueDescriptors,",
    replace: "    catalogueDescriptors: new Map(),",
    kills: ["the judge produces correct verdicts when PNGs are pre-verified and only JSONs stream through the source"],
  },
  ...
  // ------------------------------------------------- step-JSON exclusion
  {
    ...
    breaks:
      ...
      "in engineRead instead of hash-verify-only — the same classification error as the original defect",
    file: INPUTS,
  ...
  // ------------------------------------------------- walker session leaf
  {
    name: "the -observation.json disjunct is removed — unfamiliar path families never produce descriptors",
    breaks:
      ...
      "reaches the engine-read set, and the loss surfaces as a misleading CITED_ARTIFACT_MISSING",
    file: INPUTS,
    find: "  if (/-observation\\.json$/i.test(b)) return true;",
    replace: "  // MUTANT: walker session leaf disjunct removed",
    kills: ["a session from an unfamiliar path family still produces a descriptor via the walker's -observation.json leaf (SCR-2.1 dotted slug)"],
  },
];

await runMutantSuite({
  title: "A3b memory-safe judging mutants — can the bounded-residency tests still fail?",
  filter: "A3",
  mutants: MUTANTS,
});
