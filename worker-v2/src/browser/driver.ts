...
function avoidLabelMatches(optionLabel: string, avoidLabel: string): boolean {
  ...
  if (a === b) return true;
  // Rule 2: token-superset boundary — all tokens of the avoid label appear in the option
  // label AND the option label has no additional substantive tokens (allowing articles,
  // conjunctions and minor fluff). This catches forms like "(Hospital)" matching "Hospital"
  // without allowing "a group of hospital(s) and clinic(s)" to match "Hospital".
