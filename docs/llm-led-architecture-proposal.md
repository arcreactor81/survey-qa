...
1# LLM-Led Survey Link Testing — Architecture Proposal
...
| 4 | Agent topology | Logical **planner → executor → verifier** separation; the executor never certifies its own success. **Execute each path once; verify in layers:** deterministic checks and a single verifier for routine passes; a family-diverse **three-model panel** independently reviews checklist extraction, every asserted defect, high-risk/inconclusive items, a stratified sample of passes, and the final synthesis — all from the shared immutable evidence. Panelists vote independently before bounded reconciliation; unresolved dissent stays visible as `inconclusive` and may trigger one targeted re-check — never three parallel executions. (This carries phase-1's proven N-of-3 consensus machinery forward at the judgment layer, where it's cheap, instead of the execution layer, where it triples the dominant cost.) |
...
**Cost containment — the #1 lever is a coverage-aware hard budget, not model choice.** Tiering and caching lower *average* cost; only a hard budget bounds the *worst case*, and the worst case is what "spirals". The tiering default is an orchestrator–worker split: a strong "overseer" model plans paths and adjudicates evidence at checkpoints, while a cheap, fast model does the actual clicking and form-filling — escalating to the overseer only on stall, ambiguity, or expected-vs-observed mismatch (management by exception; per-click review by the strong model would re-spend the savings). Which models fill the seats is a bakeoff question for P1, using the phase-1 bakeoff methodology — judged on cost per *verified coverage unit* (not sticker price), structured-output/tool-call reliability, grounding accuracy (text-only models need accessibility-tree navigation or a vision proxy where screenshots matter), and latency. Panel-adjudication spend is metered separately inside the verification reserve (a P1 hypothesis: two extra reviews plus bounded reconciliation), panel-triggered re-checks are capped, and sharing evidence with multiple model providers requires the same clearance as any external service. Proposed starting guardrails (explicit hypotheses, recalibrated after Phase 1):
...
| LLM verdicts wrong on semantics | Deterministic assertions first; layered verification with the three-model panel on extraction, asserted defects, and synthesis; executor never self-certifies |
| Shared-evidence blindness (panel agrees on wrong/incomplete evidence) | Panelists receive artifact references + raw excerpts, never only executor-written summaries; votes are independent before reconciliation; dissent preserved as `inconclusive`; stratified pass-sampling catches false negatives, not just false positives |
